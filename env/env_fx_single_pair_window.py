...
class tgym(gym.Env):
    ...
    def __init__(self, df, env_config_file="./env/config/gdbusd-test-9.json") -> None:
        ...
        self.observation_space = spaces.Box(
            high=60.0, low=0.0, shape=(self.backward_window, len(self.observation_list))
        )
        print(
            f"initial done:\n"
    ...
    def _take_action(self, action, done):
        if not self.training:
            ...
            self._o, self._c, self._h, self._l, self._t, self._day = self.df.iloc[
                self.current_step
            ][["Open", "Close", "High", "Low", "_time", "_day"]]
            ...
            if (
                action in (ActionEnum.BUY, ActionEnum.SELL)
                and not done
                and self.current_holding
                < self.cf.symbol(self.asset, "max_current_holding")
            ):
                self.ticket_id += 1
                if self.cf.symbol(self.asset, "limit_order"):
    ...
    def _limit_order_process(self, _action, done):
        for tr in self.transaction_limit_order:
            if tr["Type"] != _action or done:
                ...
                if (tr["ActionPrice"] >= self._l and _action == 0) or (
                    ...
                    tr["LimitStep"]
                    + self.cf.symbol(self.asset, "limit_order_expiration")
                    > self.current_step
                ):
                    tr["CloseStep"] = self.current_step
                    tr["Status"] = 4
    ...
    def step(self, action):
        ...
        if not self.training:
            ...
            if self.balance != 0:
                self.max_draw_down_pct = abs(self.max_draw_downs / self.balance * 100)
    ...
    def get_observation(self):
        if self.current_step - self.backward_window < 0:
            ...
            _d = self.df.iloc[
                self.current_step - self.backward_window : self.current_step
            ]
